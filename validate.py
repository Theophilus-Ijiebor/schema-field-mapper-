"""
JSON-schema validation for the final mapping document.

Guards against the pipeline ever silently emitting output that doesn't match
the structure the assignment specifies (missing keys, wrong types, etc.).
"""
...
def validate_coverage(doc: dict, expected_field_counts: dict[str, int]) -> list[str]:
    """
    Sanity check that every source field is accounted for exactly once
    (either mapped or listed as unmapped) per table. Returns a list of
    human-readable problems; empty list means clean.
    """
    problems = []
