...
FK references, and the inline comment.

Nothing in this module talks to an LLM, a database, or a tenant. It is pure,
tenant-agnostic reference data -- the same two schemas are shared by every
tenant in this deliverable. (A multi-schema-pair product would swap this
module for a per-tenant schema registry; see app/tenancy/registry.py for
where that seam would live.)
"""
